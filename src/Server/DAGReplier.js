...
(function(COMPOSITE, DAGUtil, SocketUtil) {
    ...
    DAGReplier.prototype.add = function(request) {
        if (request.verb ===undefined || request.subtree === undefined) {
            return request;
        }
        ...
        for (var i = 0 ; i < results.length ; i++) {
            SocketUtil.sendNode(request.var.socket, results[i]);
        }
...
})(COMPOSITE, COMPOSITE.DAGUtil, COMPOSITE.SocketUtil)
